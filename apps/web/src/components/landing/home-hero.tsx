...
import { GoldExpandLine, RevealImage, RevealOnMount, Stagger, StaggerItem } from "@/components/landing/reveal";

/** Hero editorial: texto + CTAs → foto rooftop (mobile); 55/45 assimétrico no desktop. */
    ...
    <section className="relative overflow-hidden bg-navy-950">
      <div className="mx-auto grid max-w-7xl lg:min-h-[100svh] lg:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)]">
        <div className="relative z-10 flex flex-col justify-center px-5 pb-8 pt-24 sm:px-8 sm:pt-28 lg:px-10 lg:pb-16 lg:pt-20">
          <Stagger onMount className="flex max-w-xl flex-col">
            <StaggerItem>
            ...
            <StaggerItem>
              <div className="mt-12 flex flex-col gap-4 lg:mt-14">
                <GoldExpandLine onMount className="w-16" />
                <RevealOnMount delay={0.35}>
                  <p className="max-w-sm text-[0.75rem] leading-relaxed tracking-[0.02em] text-slate-400">
                    Formações estruturadas com processo, método e aplicação —
                    para quem decide operar com mais clareza.
                  </p>
                </RevealOnMount>
              </div>
            </StaggerItem>
        ...
        <div className="relative mt-4 h-[72vw] max-h-[480px] min-h-[300px] w-full sm:mt-6 lg:mt-0 lg:h-auto lg:max-h-none lg:min-h-[100svh]">
          <RevealImage onMount className="absolute inset-0 h-full w-full" delay={0.1}>
            <EditorialPhoto
              src={photo.src}
              alt={photo.alt}
